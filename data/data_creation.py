        ...
        for github_var in github_list:
            for var in row[github_var]:
                ...
                if commit in ["compare", "blob"]:
                    logger.debug(f"Unable to parse {var} - no compare in url")
                    continue

                if commit not in ["commit", "commits"]:
                    logger.debug(f"Unable to parse {var} - no commit or commits in url")
                    continue
...
def aggregate_all(output_dir):
    ...
    for repo, vuln_commits in list(repo_commits.items())[:]:
        ...
        with open(
            os.path.join(output_dir, json_commit_directory, f"{repo_real_name}.json"),
            "r",
        ) as fin:
...
def extract_commits_from_projects(output_dir):
    ...
    for repo_name in repo_commits.keys():
        ...
        for commit in commits:
            ...
            time = datetime.utcfromtimestamp(commit["committer"]["date"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        ...
        with open(
            os.path.join(output_dir, json_commit_directory, f"{author}_{repo}.json"),
            "w",
        ) as fout:
            json.dump(res, fout, indent=4)
        safe_mkdir(os.path.join(output_dir, timezone_directory))
        with open(
            os.path.join(output_dir, timezone_directory, f"{author}_{repo}.json"), "w"
        ) as fout:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cve", action="store_true", help="Runs cve preprocessing")
    parser.add_argument(
        "--graphql", action="store_true", help="Runs graphql preprocessing"
    )
    parser.add_argument(
        "--metadata", action="store_true", help="Stores metadata of repository"
    )
